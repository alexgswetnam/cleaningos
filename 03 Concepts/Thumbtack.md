---
type: concept
engine: [Leads]
season: [Survival, Stability]
laws: [Stop Guessing]
status: Canonical
sources: [2025-02-03 Weekly Coaching Call, Free Thumbtack Leads + GBP Optimization, Free Resources and Mini Courses]
updated: 2026-08-06
---

# Thumbtack

> [!abstract] In one line
> Profitable if you filter hard and call fast — expensive if you let it send you anything.

## Definition

Lead marketplace. Second-most-profitable channel in Alex's [[Channel Prioritization]]
ranking, largely because of the free-leads method.

## Why It Matters

Unfiltered, it will happily sell you one-bed one-bath one-time jobs at full price. Filtered
to recurring work with the price slider worked up from the bottom, it performs.

Alex's Q4: 90 leads, **51 of them free**, $15.64 cost per lead, 26% close, $58 CPA.

## Symptoms

- Spend spiking on leads that don't fit your business
- Leads that never answer
- Low close rate specifically from this channel

## Common Mistakes

- Not filtering out unprofitable job types. One member spent **$220 in a day on four bad
  leads** — 1-bed/1-baths and one-times.
- Starting the lead price slider at the top instead of working up from the bottom
- Not using the free-leads cycling method
- **Only texting.** Call, and call more than once.

## Models

**The follow-up sequence:**

1. **Message inside Thumbtack immediately** — they measure your response time
2. **Call.** Twice. Three times if it looks like a serious job (not a 1-bed/1-bath)
3. **Leave a voicemail**
4. **Text:** *"Hey, I just tried you. You just reached out. When's a better time to call?"*
5. Hand ongoing follow-up to the CRM

Alex on calling three times: *"I have no shame... you reached out to me and said you want
cleaning services."*

**If they push back on the calls:**

> "Am I bothering you? You just reached out to us and I wanted to get ahold of you."

And if they stay hostile — they probably weren't going to be a good client.

**The free-leads method, end to end.** Thumbtack is the one major channel where you can
specifically target recurring work — not available on PPC or Facebook — which is why
filtering matters so much here. Two filters first: job type (remove move-outs,
post-construction, and vacation-rental cleans — they don't make money) and frequency
(recurring only, never "just once"). Then the budget mechanism that generates free leads:

1. Set the Thumbtack budget setting itself to **"unlimited."**
2. Put Thumbtack on a **dedicated prepaid card**, loaded with only what you actually want
   to spend for the period (e.g. $100–$400).
3. When the card runs dry, Thumbtack can't charge more — it shows a payment failure — but
   because the budget *setting* is still "unlimited," Thumbtack periodically offers
   **batches of free direct leads** to keep you engaged (the example: 21 free leads
   starting the next day, worth roughly $1,800 at ~$60/lead).
4. Accept the offer, receive the free leads, and let the card run dry again. The cycle
   repeats — usually within a day or two of the card emptying.

A second, separate hack: when Thumbtack prompts *"expand your service area to get free
leads,"* add the requested city to unlock the offer, then immediately change the service
area back to normal. The free leads still arrive.

**The early, expensive lesson behind all of this:** one business spent $6K over two months
on Thumbtack before realizing about $4K of it went to unprofitable one-time cleans — the
filtering and budget-cycling setup above is what turned it into a strong channel instead
of a leak.

**No-contact rate is a metric.** Q4: 6 of 90 Thumbtack leads never responded; 5 of 36 PPC
leads never responded. A high number means something upstream is wrong — it's how Alex
found the Google Ads *search network partners* setting was generating junk. See
[[PPC Search Not Display]].

## Checklist

- [ ] Filter to recurring work
- [ ] Remove move-out, post-construction, and vacation-rental job types
- [ ] Price slider from the bottom up
- [ ] Free-leads method enabled: budget setting "unlimited," spend capped via a dedicated
      prepaid card
- [ ] When the card empties, wait 1–2 days for a free-lead batch offer, then accept it
- [ ] Optionally cycle the "expand service area" free-lead prompt, then revert the area
- [ ] Message in-platform the moment a lead lands
- [ ] Call twice, three times if serious
- [ ] Voicemail, then text
- [ ] CRM takes over
- [ ] Track no-contact rate

## Templates

- "Hey, I just tried you. You just reached out. When's a better time to call?"
- "Am I bothering you? You just reached out to us and I wanted to get ahold of you."

## Videos

[[Free Thumbtack Leads + GBP Optimization]] — the full free-leads method walkthrough.
[[Free Resources and Mini Courses]] — "$100 Bonus - How to Get FREE Leads from
Thumbtack" lesson.

## Student Examples

**Rick** — $220 spend, 4 leads, $55 cost per lead, 25% close. Unfiltered. See
[[Marketing Math]] for the full breakdown.

**The host** — $6K spent over two months early on, ~$4K of it wasted on unprofitable
one-time cleans, before the job-type/frequency filters and free-lead cycling turned
Thumbtack into a genuinely strong channel.

**A second, independent live demo confirms the exact same card-cycling method** — set
Thumbtack's budget to "unlimited," fund an actual spend cap with a prepaid/gift card
loaded only with the amount you want to spend, and keep the unlimited setting on so free
leads keep triggering. Alex estimates having received **250–300 free leads total** this
way, concentrated in the business's second and third months. Also reconfirms the
service-area-expansion trick from a slightly different angle: opting into suggested
areas to trigger a free-lead offer (the example: ~30 free leads), then removing those
areas afterward so ongoing leads stay local.

## AI Prompts

*None yet.*

## FAQ

**Q:** Isn't calling three times aggressive?
**A:** They submitted a request for cleaning services. Alex's position: no shame.

## Conflict History

*None.*

## Presented In

*None yet.*

## Related Concepts

- [[Channel Prioritization]]
- [[Marketing Math]]
- [[Close Rate By Channel]]
- [[PPC Search Not Display]]
- [[Word Tracks]]
